...
export default function Home() {
  ...
  const handleSearch = async (url: string) => {
    // Si el usuario le da a buscar sin link, "volvemos atrás" limpiando el video actual
    if (!url.trim()) {
      setMetadata(null);
      setVideoUrl("");
      setError(null);
  ...
  return (
    <main className="min-h-screen relative flex flex-col items-center py-12 px-4 md:px-24 overflow-x-hidden">
      <Background />
      ...
      >
        <h1 className="text-6xl md:text-8xl font-extrabold mb-4 tracking-tighter glow-text">
        ...
        {recentVideos.length > 0 && !metadata && !isLoading && (
          ...
          >
            <div className="flex items-center justify-between mb-4 px-2">
              <div className="flex items-center gap-2 text-white/40 font-medium uppercase tracking-widest text-xs">
                <History className="w-4 h-4" />
                Recent Searches
              </div>
              <button 
                onClick={clearHistory}
                className="text-white/20 hover:text-red-400 transition-colors text-xs flex items-center gap-1"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {recentVideos.map((video, idx) => (
                ...
                >
                  <img src={video.thumbnail} className="w-20 h-12 object-cover rounded-lg opacity-60 group-hover:opacity-100 transition-opacity" />
                  <div className="flex-1 min-w-0">
                    <p className="text-white/80 font-medium text-sm truncate">{video.title}</p>
                    <p className="text-white/30 text-xs truncate">{video.author}</p>
                  </div>
      ...
      <motion.footer 
        ...
        animate={{ opacity: 1 }}
        transition={{ delay: 1 }}
        className="mt-24 text-white/20 text-xs font-medium tracking-widest uppercase z-10"
      >
        Premium Experience • Next.js & Youtube Master API
      </motion.footer>
    </main>
  );
}
